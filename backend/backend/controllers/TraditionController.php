...
use app\models\TraditionSearch;
...
class TraditionController extends Controller
{
    ...
    {
        $searchModel = new TraditionSearch();
    ...
    {
        $query = new Query();
        $data = $query->select('*')->from('tradition')
            ->innerjoin('global_tradition', 'global_tradition.global_tradition_id = tradition.global_tradition_id')
            ->innerJoin('community', 'community.community_id = tradition.community_id')
            ->where(['tradition.tradition_id' => $id])
            ->all();
    ...
    public function actionCreate()
    {
        ...
        if ($model->load(Yii::$app->request->post())) {
            ...
            //save image
            $model->tradition_image_cover = Yii::$app->Upload->Importimage($model, 'tradition', 'uploads/community/' . $model->community_id . '/tradition/', 'tradition_image_cover');
            ...
            if ($model->save()) {
                ...
                return $this->redirect(['index', 'id' => $model->tradition_id]);
            }
        }
    ...
    public function actionUpdate($id)
    {
        ...
        $image_old = $model->tradition_image_cover;
        ...
        if ($model->load(Yii::$app->request->post())) {
            ...
            //save image
            $model->tradition_image_cover = Yii::$app->Upload->Updateimage($image_old, $model, 'tradition', 'uploads/community/' . $model->community_id . '/tradition/', 'tradition_image_cover');
    ...
    public function actionDelete($id)
    {
        ...
        if (!empty($model)) {
            @unlink(Yii::getAlias('@webroot/uploads/community/' . $model->community_id . '/tradition/' . $model->tradition_image_cover));
        }
    ...
    {
        if (($model = Tradition::findOne($id)) !== null) {
            return $model;
        }
