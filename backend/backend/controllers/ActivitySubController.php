...
use app\models\ActivitySubSearch;
...
class ActivitySubController extends Controller
{
    ...
    {
        $searchModel = new ActivitySubSearch();
    ...
    {
        $query = new Query();
        $data = $query->select('*')->from('activity_sub')
                ->innerJoin('activity', 'activity.activity_id = activity_sub.activity_id')
                ->where(['activity_sub_id' => $id])->all();
    ...
    public function actionCreate($activity_id)
    {
        ...
        if ($model->load(Yii::$app->request->post())) {
            ...
            if($model->save()){
                ...
                //return $this->redirect(['index', 'id' => $model->activity_sub_id]);
    ...
    {
        if (($model = ActivitySub::findOne($id)) !== null) {
            return $model;
        }
